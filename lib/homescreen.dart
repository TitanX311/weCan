...
class _HomescreenState extends State<Homescreen> {
  ...
  void _onTabTapped(int index) {
    ...
    switch (index) {
      case 0:
        // Handle navigation for Home (or stay on the same screen)
        setState(() {
          _currentIndex = index;
        });
        break;
        ...
        children: [
          ...
          StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              final base64Images =
                  docs.map((doc) => doc['image_data'] as String).toList();
...
class TodayCard extends StatelessWidget {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || !snapshot.data!.exists) {
          return _buildCard(
              context, day.toUpperCase(), "No Volunteers assigned", [], null);
        }
  ...
  Widget _buildCard(BuildContext context, String title, String subtitle,
      List<String> volunteers, DocumentReference? reference) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
